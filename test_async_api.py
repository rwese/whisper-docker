...
class TestAsyncAPI:
    ...
    def test_submit_async_task(self):
        ...
        with open(self.test_file.name, "rb") as f:
            ...
            data = {"model": "tiny", "language": "en", "output_format": "json"}

            response = requests.post(
                f"{self.BASE_URL}/transcribe/async", files=files, data=data
            )
    ...
    def test_get_task_result_different_formats(self):
        ...
        for format_type in formats_to_test:
            response = requests.get(
                f"{self.BASE_URL}/tasks/{task_id}/result?format={format_type}"
            )
            assert response.status_code == 200
    ...
    def test_pending_task_result(self):
        ...
        if response.status_code == 202:
            ...
            assert (
                "pending" in error_data["detail"]
                or "processing" in error_data["detail"]
            )
