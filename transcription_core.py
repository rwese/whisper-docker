        ...
        if self.model is None:
            with self._model_lock:
                ...
                if self.model is None:
                    ...
                    print(
                        f"[{datetime.now(timezone.utc).isoformat()}] Loading Whisper model: {self.model_name}",
                        flush=True,
                    )
                    ...
                    if STRUCTLOG_AVAILABLE:
                        logger.info(
                            "Whisper model loaded successfully", model=self.model_name
                        )
                    else:
                        logger.info(
                            f"Whisper model loaded successfully: {self.model_name}"
                        )
                    print(
                        f"[{datetime.now(timezone.utc).isoformat()}] Whisper model loaded: {self.model_name}",
                        flush=True,
                    )
        return self.model
