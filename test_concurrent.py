...
def create_test_audio():
    ...
    if not Path(TEST_AUDIO_FILE).exists():
        print(f"Warning: {TEST_AUDIO_FILE} not found. Please provide a test audio file.")
        return False
    return True
...
async def test_single_request(session, request_id, model="tiny"):
    ...
    try:
        ...
        with open(TEST_AUDIO_FILE, 'rb') as f:
            ...
            async with session.post(f"{BASE_URL}/transcribe/async", data=data, timeout=TIMEOUT) as response:
                if response.status == 200:
                    ...
                    for i in range(30):  # Wait up to 30 seconds
                        ...
                        async with session.get(f"{BASE_URL}/tasks/{task_id}") as status_response:
                            if status_response.status == 200:
                                status_data = await status_response.json()
                                
                                if status_data['status'] == 'completed':
                                    # Get result
                                    async with session.get(f"{BASE_URL}/tasks/{task_id}/result") as result_response:
                                        if result_response.status == 200:
                                            ...
                                            return {
                                                ...
                                                'processing_duration': status_data.get('processing_duration_seconds', 0)
                                            }
                                        else:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for result in results:
            if isinstance(result, Exception):
                ...
                print(f"Request {result['request_id']}: Success in {result['duration']:.2f}s, "
                ...
                failed += 1
                print(f"Request {result['request_id']}: Failed in {result['duration']:.2f}s - {result['error']}")
        ...
        print(f"  Average request time: {total_transcription_time / successful:.2f}s" if successful > 0 else "  No successful requests")
        print(f"  Concurrency benefit: {(total_transcription_time / total_duration):.2f}x" if total_duration > 0 else "  N/A")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            with open(TEST_AUDIO_FILE, 'rb') as f:
                ...
                async with session.post(f"{BASE_URL}/transcribe/async", data=data) as response:
                    if response.status == 200:
                        task_info = await response.json()
                        task_id = task_info['task_id']
                                ...
                                if status_response.status == 200:
                                    ...
                                    if status_data['status'] == 'completed':
                                        ...
                                        async with session.get(f"{BASE_URL}/tasks/{task_id}/result") as result_response:
                                            if result_response.status == 200:
                                                result = await result_response.json()
                                                print(f"Async result: {len(result.get('text', ''))} characters")
                                                print(f"Processing time: {status_data.get('processing_duration_seconds', 0):.2f}s")
                                                break
                                    elif status_data['status'] == 'failed':
                                        print(f"Async task failed: {status_data.get('error_message', 'Unknown error')}")
                                        break
                        else:
                            print("Async task did not complete in time")
                    else:
                        error_text = await response.text()
                        print(f"Failed to submit async task: {response.status} - {error_text}")
